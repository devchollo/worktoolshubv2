...
class AuthUtils {
    ...
    if (container) {
      container.innerHTML = `
        <div style="display: flex; align-items: center; gap: 1rem; padding: 1rem; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
          <img src="${this.user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(this.user.name)}`}" 
               alt="${this.user.name}" 
  ...
  async initToolAuth(toolName, toolDescription = null, options = {}) {
    ...
    try {
      const toolInfoResponse = await fetch(`${this.API_BASE}/auth/tool-info/${toolName}`);
